        ...
        if (item == null) {
            if (mBrowser.canABrowseStartPlayback()) {
                throw AssertionError(
                    "Could not park the player. Its queue is empty and the service holds " +
                        "active station ${mBrowser.activeStationId()}, so the next page-0 " +
                        "browse would download new stations and play them, and this fixture " +
                        "seeded nothing to put back in the queue."
                )
            }
